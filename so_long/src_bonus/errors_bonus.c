...
/*   Updated: 2022/10/13 10:05:22 by avila-ca         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (++j < game->height - 1)
	{
		while (++i <= game->width)
		{
			...
			if (game->map[j][i] != '0' && game->map[j][i] != '1'
				...
				&& game->map[j][i] != 'P' && game->map[j][i] != 'S'
				&& game->map[j][i] != '\0')
				return (0);
			ft_check_enemies(game, j, i);
		}
		i = -1;
	}
	if (items == 2 && game->col > 0 && game->num_enemy > 0)
		return (items);
	return (0);
}
